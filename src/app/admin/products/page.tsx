...
function ProductsContent() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Selection banner */}
      {someSelected && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {confirmBulkDelete ? (
              <div className="flex items-center gap-2">
                <span className="text-red-600 text-sm font-medium">Delete {selectedIds.size} products?</span>
                <button onClick={handleBulkDelete} disabled={deletingBulk}
                  className="px-4 py-1.5 rounded-lg bg-red-500 hover:bg-red-600 text-white text-sm font-bold disabled:opacity-50">
              ...
              <button onClick={() => setConfirmBulkDelete(true)}
                className="px-4 py-1.5 rounded-lg bg-red-500 hover:bg-red-600 text-white text-sm font-bold">
                🗑️ Delete Selected
              </button>
            )}
          </div>
      ...
      {!loading && filtered.length > 0 && (
        <>
          ...
          <div className="hidden md:block bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map(p => (
                  <tr key={p.id} className={`hover:bg-gray-50/60 ${!p.inStock ? 'opacity-50' : ''} ${selectedIds.has(p.id) ? 'bg-red-50/40' : ''}`}>
                    {/* Checkbox */}
          ...
          <div className="md:hidden space-y-3">
            {filtered.map(p => (
              <div key={p.id} className={`bg-white rounded-2xl border shadow-sm p-4 ${!p.inStock ? 'opacity-60' : ''} ${selectedIds.has(p.id) ? 'border-red-300 bg-red-50/30' : 'border-gray-100'}`}>
